...
| E03-T1 | sonnet | 1 | ⏳ dispatched | — | Valhalla-Pipeline (Welle 2a). build-tiles.sh + atomischer Swap (W-17), valhalla.json service_limits Mini-PC, Compose→gis-ops (ADR-014), CI-Job valhalla-li-build. Verifikation NUR über CI (lokal: Docker aus + geofabrik geblockt). User-Mandat: autonom weiter ohne Rückfrage je Task |
