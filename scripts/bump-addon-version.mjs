 ...
 * `ha-addon/README.md` (E08-T4) hält bereits fest, dass das eigenständige
 * `yapaja-go-ha-addon`-Repository (das Format, in dem HA-Nutzer eine
 * "Add-on-Repository"-URL eintragen, docs/04-home-assistant.md §3) ein
 * SPÄTERER Spiegelungs-Schritt ist ("ein `git subtree`/Mirror-Schritt, keine
 * Code-Änderung") -- dieses Monorepo hier (`ha-addon/yapaja_go/`) ist die
 * PACKAGING-QUELLE. Dieses Skript bumpt `ha-addon/yapaja_go/config.yaml`s
 * `version:`-Feld -- die maßgebliche Stelle, die der HA-Supervisor beim
 * Update tatsächlich liest. Der Push in ein separates
 * `yapaja-go-ha-addon`-Repo (sobald es existiert) ist ein zusätzlicher
 * Mirror-Schritt, den `.github/workflows/release.yml` dokumentiert, aber
 * NICHT ausführt -- diese Entwicklungsumgebung hat weder Netzwerkzugriff
...
export const ADDON_CONFIG_PATH = join(REPO_ROOT, 'ha-addon', 'yapaja_go', 'config.yaml');
...
function main() {
  ...
  if (before === after) {
    console.log(`ha-addon/yapaja_go/config.yaml ist bereits auf Version ${newVersion}.`);
    return;
  }
  writeFileSync(ADDON_CONFIG_PATH, after, 'utf-8');
  console.log(`ha-addon/yapaja_go/config.yaml: version -> "${newVersion}"`);
}
